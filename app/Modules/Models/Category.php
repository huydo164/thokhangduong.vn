...
class Category extends Model{
    ...
    protected $table = CDatabase::category;
    ...
    protected $fillable = array(
        ...
        'category_menu', 'category_created','category_order_no', 'category_status', 'category_image',
    ...
    public static function searchByCondition($dataSearch=array(), $limit=0, $offset=0, &$total){
        try{
            ...
            if (isset($dataSearch['category_hot']) && $dataSearch['category_hot'] != -1) {
                $query->where('category_hot', $dataSearch['category_hot']);
            }
    ...
    public static function deleteId($id=0){
        try {
            ...
            if($data != null){
                ...
                $category_image_small = ($data->category_image_small != '') ? $data->category_image_small : '';
                if($category_image_small != ''){
                    $path = Config::get('config.DIR_ROOT').'uploads/'.CGlobal::FOLDER_CATEGORY.'/'.$id;
                    if(is_file($path.'/'.$data->category_image_small)){
                        @unlink($path.'/'.$data->category_image_small);
                    }
                    if(is_dir($path)) {
                        @rmdir($path);
                    }
    ...
    public static function getArrCategoryByTypeId($typeid=0, $parent=false){
        $result = [];
            ...
            foreach($arrCate as $cate){
                if($parent == true){
                    if($cate->category_parent_id == 0){
                        $result[$cate->category_id] =  stripcslashes($cate->category_title);
                    }
                }else{
                    $result[$cate->category_id] =  stripcslashes($cate->category_title);
                }
